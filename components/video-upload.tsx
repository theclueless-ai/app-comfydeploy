...
export function VideoUpload({
  ...
  const handleFile = (file: File | null) => {
    ...
    if (file.size > maxSize) {
      alert(`File is too large (${formatBytes(file.size)}). Please use a video smaller than ${maxSizeMB}MB.`);
      return;
    }
    ...
    <div className="w-full">
      ...
      >
        ...
        {value ? (
          ...
          <label
            htmlFor={`upload-video-${label}`}
            className="flex items-center justify-center cursor-pointer py-1 gap-2"
          >
            <Film className="w-4 h-4 text-[rgb(var(--muted-foreground))]" />
            <p className="text-xs text-[rgb(var(--muted-foreground))]">
